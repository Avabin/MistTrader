...
public class ProxyViewModelTests
{
    ...
    private Subject<ResponseCaptured> _responseCapturedSubject = null!;
    ...
    public void Setup()
    {
        ...
        _responseCapturedSubject = new Subject<ResponseCaptured>();
        
        _messageBus.Listen<ProxyStarted>().Returns(_proxyStartedSubject);
        _messageBus.Listen<ProxyStopped>().Returns(_proxyStoppedSubject);
        _messageBus.Listen<ResponseCaptured>().Returns(_responseCapturedSubject);

        _viewModel = new ProxyViewModel(_mediator, _messageBus, _messageBoxService);
    ...
    {
        // Act & Assert
        var act = () => new ProxyViewModel(null!, _messageBus, _messageBoxService);
    ...
    {
        // Act & Assert
        var act = () => new ProxyViewModel(_mediator, null!, _messageBoxService);
        act.Should().Throw<ArgumentNullException>()
    ...
    public async Task OnResponseCaptured_ShouldIncrementCapturedResponses()
    {
        ...
        // Act
        _responseCapturedSubject.OnNext(new ResponseCaptured(new Uri("http://localhost"), "Test response", DateTimeOffset.Now));
    ...
    {
        // Arrange
        var response = new ResponseCaptured(new Uri("http://localhost"), "Test response", DateTimeOffset.Now);
    ...
    public async Task OnResponseCaptured_ShouldAddMultipleResponsesToCapturedResponses()
    {
        ...
        var response1 = new ResponseCaptured(new Uri("http://localhost"), "Test response 1", baseTime);
        var response2 = new ResponseCaptured(new Uri("http://localhost"), "Test response 2", baseTime.AddSeconds(1));
